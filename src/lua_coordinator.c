...
//#define LUA_BLOCK_FILE 		"/home/mk/prog/c/microblx/std_blocks/lua_coordinator/lua_coordinator.lua"
...
char lua_coordinator_meta[] =
	"{ doc='A generic luajit based block',"
	"  license='MIT',"
	"  real-time=false,"
	"}";

struct lua_coordinator_info {
...
{
	int ret = 0;
	struct lua_coordinator_info* inf = (struct lua_coordinator_info*) b->private_data;
 ...
 */
static int init_lua_state(struct lua_coordinator_info* inf, const char* lua_file, const char* lua_str)
{
	int ret=-1;
...
static int lua_coordinator_init(ubx_block_t *b)
{
	char *lua_file, *lua_str;
	unsigned int lua_file_len, lua_str_len;

	int ret = -EOUTOFMEM;
	struct lua_coordinator_info* inf;

	if((inf = calloc(1, sizeof(struct lua_coordinator_info)))==NULL)
		goto out;
...
static int lua_coordinator_start(ubx_block_t *b)
{
	return call_hook(b, "start", 0, 1);
}

/**
 * lua_coordinator_step - execute lua string and call step hook
 *
 * @param b
 */
static void lua_coordinator_step(ubx_block_t *b)
{
	int len=0, ret;
	struct lua_coordinator_info* inf = (struct lua_coordinator_info*) b->private_data;
...
static void lua_coordinator_stop(ubx_block_t *b)
{
	call_hook(b, "stop", 0, 0);
}

static void lua_coordinator_cleanup(ubx_block_t *b)
{
	struct lua_coordinator_info* inf = (struct lua_coordinator_info*) b->private_data;
...
ubx_block_t lua_comp = {
	.name = "lua/lua_coordinator",
	.type = BLOCK_TYPE_COMPUTATION,
	.meta_data = lua_coordinator_meta,
	.configs = lua_conf,
	.ports = lua_ports,

	/* ops */
	.init = lua_coordinator_init,
	.start = lua_coordinator_start,
	.step = lua_coordinator_step,
	.stop = lua_coordinator_stop,
	.cleanup = lua_coordinator_cleanup,
};
...
{
	ubx_block_unregister(ni, "lua/lua_coordinator");
}
